...
var allDates = d3.map(data, function(d){return d.date;}).keys();
allDates.forEach(function(d, i) { allDates[i] = parseTime(d); })
var allSites = d3.map(data, function(d){return d.site;}).keys();


var newdata = [];
allSites.forEach(function(d) {
  newdata.push({"site":d, "values":[]}); 
});

data.forEach(function(d) {
   newdata[allSites.indexOf(String(d.site))].values.push({"date":d.date, "value":d.value});
});
...
if(options.hasOwnProperty("threshold")) {
    var tdata = [{
        ...
        "line":[{"x":20, "y":allSites.length*20+20}, {"x":70, "y":allSites.length*20+20}]
    }];
...
var filename = options.loc + "_" + options.metric + "_siteplot.png";
